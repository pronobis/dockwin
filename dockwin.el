...
the given window.  ORIG-FUN and ARGS are advice arguments."
  (let* ((win-from (selected-window))
    ...
    (let ((w (dockwin--get-bottom-window win-from-frame)))
      (when (and (window-live-p w) (not (eq win-to w)) (eq win-from w))
        (enlarge-window (- dockwin-bottom-height-min (window-height)))))
    (let ((w (dockwin--get-top-window win-from-frame)))
      (when (and (window-live-p w) (not (eq win-to w)) (eq win-from w))
        (enlarge-window (- dockwin-top-height-min (window-height)))))
    (let ((w (dockwin--get-left-window win-from-frame)))
      (when (and (window-live-p w) (not (eq win-to w)) (eq win-from w))
        (enlarge-window (- dockwin-left-width-min (window-width)) t)))
    (let ((w (dockwin--get-right-window win-from-frame)))
      (when (and (window-live-p w) (not (eq win-to w)) (eq win-from w))
    ...
    (let ((w (dockwin--get-bottom-window win-to-frame)))
      (when (and (window-live-p w) (not (eq win-from w)) (eq win-to w))
        (enlarge-window (- dockwin-bottom-height-max (window-height)))
        (setq recenter t)))
    (let ((w (dockwin--get-top-window win-to-frame)))
      (when (and (window-live-p w) (not (eq win-from w)) (eq win-to w))
        (enlarge-window (- dockwin-top-height-max (window-height)))
        (setq recenter t)))
    (let ((w (dockwin--get-left-window win-to-frame)))
      (when (and (window-live-p w) (not (eq win-from w)) (eq win-to w))
        (enlarge-window (- dockwin-left-width-max (window-width)) t)
        (setq recenter t)))
    (let ((w (dockwin--get-right-window win-to-frame)))
      (when (and (window-live-p w) (not (eq win-from w)) (eq win-to w))
